const SHA256 = require('crypto-js/sha256');

class Block{

    //블록의 구성 요소
    constructor(index, timestamp, data, previousHash = ''){ 
        this.index = index;
        this.timestamp = timestamp;
        this.data = data;
        this.previousHash = previousHash;
        this.hash = this.calculateHash();
    }
    //요소를 모두 넣으면 해시값이 나온다.
    calculateHash(){
        return SHA256(this.index + this.previousHash + this.timestamp + JSON.stringify(this.data)).toString(); 
    }
}

class Blockchain{
    //블록체인 관련 클래스
    constructor(){
        this.chain = [this.createGenesisBlock()];
    }
    //생성할 때 Block 객체를 만들면서 시작 -> 제네시스 블록 생성

    createGenesisBlock(){
        return new Block(0,"09/01/2020","GenesisBlock","0");
    }
    // 최신 블록 가져오는 함수
    getLatestBlock(){
        return this.chain[this.chain.length - 1];
    }

    //블록 더하는것
    addBlock(newBlock){
            newBlock.previousHash = this.getLatestBlock().hash;
            newBlock.hash = newBlock.calculateHash();
            this.chain.push(newBlock);
    }

    //유효성 검사
    isChainValid(){
        for(let i =1; i < this.chain.length; i++){ //모든 블록을 반복문을 통해 체크
            const currentBlock = this.chain[i];
            const previousBlock = this.chain[i-1];
                //해시를 다시 해서 정보가 조작되었는지 확인
                if(currentBlock.hash!==currentBlock.calculateHash()){
                    return false;
                }
                //이전 블록의 해시값을 다시 계산하여 정보가 조작되었는지 확인
                if(currentBlock.previousHash!== previousBlock.hash){
                    return false;
                }
        }
                //조건을 다 통과 할 시 true 반환
                return true;
    }
}

let NewCoin = new Blockchain(); //새 블록체인 만들기
NewCoin.addBlock(new Block(1, "10/08/2020", {amount : 4})); //1번 블록 추가
NewCoin.addBlock(new Block(2, "12/11/2020", {amount : 10})); //2번 블록 추가


//블록체인 
// console.log(JSON.stringify(NewCoin,null,3));


// //조작이 없는경우 타당성 검사
// console.log("is blockchain valid? " + NewCoin.isChainValid());

// //조작이 있는 경우 타당성 검사
// NewCoin.chain[1].data={amount :10000};
// console.log("is blockchain valid? " + NewCoin.isChainValid());
// 똑갘은 정보로 한번 더 해시를 해보았을때.
// NewCoin.chain[1].hash=NewCoin.chain[1].calculateHash();
// console.log("is blockchain valid? " + NewCoin.isChainValid());

